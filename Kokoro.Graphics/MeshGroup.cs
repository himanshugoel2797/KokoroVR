...
namespace Kokoro.Graphics
{
    ...
    public class MeshGroup : IDisposable
    {
        ...
        {
            OpenTK.Graphics.OpenGL4.VertexAttribPointerType vAttribPtrType = 0;
            switch (vFormat)
            {
                case MeshGroupVertexFormat.X32F_Y32F_Z32F:
                    vertex_comp_cnt = 3;
                    vertex_size = 3 * sizeof(float);
                    vAttribPtrType = OpenTK.Graphics.OpenGL4.VertexAttribPointerType.Float;
                    break;
                case MeshGroupVertexFormat.X10UI_Y10UI_Z10UI_MISC2UI:
                    vertex_comp_cnt = 1;
                    vertex_size = sizeof(uint);
                    vAttribPtrType = OpenTK.Graphics.OpenGL4.VertexAttribPointerType.UnsignedInt2101010Rev;
                    break;
            }
            ...
            varray.SetBufferObject(1, uvs, 2, OpenTK.Graphics.OpenGL4.VertexAttribPointerType.Float, false);
            varray.SetBufferObject(2, normals, 2, OpenTK.Graphics.OpenGL4.VertexAttribPointerType.Short, false);
        ...
        public void FlushBuffer(IntPtrIndex idx, int offset, int size)
        {
            switch (idx)
            {
                case IntPtrIndex.Index:
                    if (index_cnt != 0)
                    {
                        indices.FlushBuffer((ulong)(offset * sizeof(ushort)), (ulong)(size * sizeof(ushort)));
                        indiceF.PlaceFence();
                    }
                    break;
                case IntPtrIndex.Normal:
                    normals.FlushBuffer((ulong)(offset * sizeof(uint)), (ulong)(size * sizeof(uint)));
                    normalF.PlaceFence();
                    break;
                case IntPtrIndex.UV:
                    uvs.FlushBuffer((ulong)(offset * 2 * sizeof(float)), (ulong)(size * sizeof(float) * 2));
                    uvF.PlaceFence();
                    break;
                case IntPtrIndex.Vertex:
                    vertices.FlushBuffer((ulong)(offset * 3 * sizeof(float)), (ulong)(size * vertex_size));
                    verticeF.PlaceFence();
                    break;
            }
        ...
        public static explicit operator int(MeshGroup g)
        {
            return g.varray.id;
        }
