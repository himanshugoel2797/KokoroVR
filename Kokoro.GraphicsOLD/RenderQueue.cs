...
namespace Kokoro.Graphics
{
    public class RenderQueue
    {
        ...
        public RenderQueue(int MaxDrawCount, bool transient)
        {
            ...
            multiDrawParams = new StorageBuffer(MaxDrawCount * 5 * sizeof(uint), false);
        }
        ...
        public void Submit()
        {
            ...
            for (int i = 0; i < RenderStates.Count; i++)
            {
                ...
                {
                    GraphicsDevice.SetRenderState(RenderStates[i]);
                ...
                for (int j = 0; j < MeshGroups[RenderStates[i]].Count; j++)
                {
                    ...
                    GraphicsDevice.SetRenderState(bkt.State); //State has been already set if ClearFramebufferBeforeSubmit is true.

                    if (bkt.meshes[0].Mesh != null)
                        GraphicsDevice.SetCurrentMeshGroup(bkt.meshes[0].Mesh.Parent);
