    ...
    public class TextureStreamer
    {
        public class TextureStream
        {
            ...
            public TextureStream(TextureStreamer owner)
            {
                ...
                BackgroundTaskManager.RegisterBackgroundTask(UpdateProgress);
                GraphicsDevice.Cleanup.Add(Dispose);
            }

            private void Upload()
            {
                GPUStateMachine.BindBuffer((OpenTK.Graphics.OpenGL4.BufferTarget)BufferUsage.PixelUnpackBuffer, pboId);
                GL.BufferData((OpenTK.Graphics.OpenGL4.BufferTarget) BufferUsage.PixelUnpackBuffer, sz >> (int)System.Math.Pow(curLevel, dims), src.GetPixelData(curLevel), BufferUsageHint.StreamRead);

                switch (dims)
                {
                    case 1:
                        GL.TextureSubImage1D(TargetTexture.id, curLevel, 0, TargetTexture.Width >> curLevel, (OpenTK.Graphics.OpenGL4.PixelFormat)src.GetFormat(), (OpenTK.Graphics.OpenGL4.PixelType)src.GetPixelType(), IntPtr.Zero);
                        break;
                    case 2:
                        GL.TextureSubImage2D(TargetTexture.id, curLevel, 0, 0, TargetTexture.Width >> curLevel, TargetTexture.Height >> curLevel, (OpenTK.Graphics.OpenGL4.PixelFormat)src.GetFormat(), (OpenTK.Graphics.OpenGL4.PixelType)src.GetPixelType(), IntPtr.Zero);
                        break;
                    case 3:
                        GL.TextureSubImage3D(TargetTexture.id, curLevel, 0, 0, 0, TargetTexture.Width >> curLevel, TargetTexture.Height >> curLevel, TargetTexture.Depth >> curLevel, (OpenTK.Graphics.OpenGL4.PixelFormat)src.GetFormat(), (OpenTK.Graphics.OpenGL4.PixelType)src.GetPixelType(), IntPtr.Zero);
                        break;
                }
                GPUStateMachine.UnbindBuffer((OpenTK.Graphics.OpenGL4.BufferTarget)BufferUsage.PixelUnpackBuffer);
        ...
        public TextureStream UploadTexture(ITextureSource src)
        {
            ...
            if (!BackgroundTaskManager.ExecuteBackgroundTasksUntil(() => (buffers.Count != 0)))
            {
