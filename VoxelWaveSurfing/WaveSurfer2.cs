...
{
    public class DirectBitmap2 : IDisposable
    {
        ...
        public DirectBitmap2(int width, int height)
        {
            Width = width;
            Height = height;
    ...
    public class WaveSurfer2
    {
        public DirectBitmap2 Image { get; private set; }
        ...
        public WaveSurfer2(int w, int h, VoxelData data)
        {
            Image = new DirectBitmap2(w, h);
            this.data = data;
            this.w = w;
            this.h = h;
        ...
        public void Draw()
        {
            ...
            unsafe
            {
                ...
                var fstm = new Frustum(viewMat, projMat, Position);

                List<Vector2> tiles = new List<Vector2>();
                //traverse tiles front to back, computing minimum and maximum screen space bounds for each tile
                for (float y = 0; y < VoxelData.WorldSide; y += VoxelData.WorldStep)
                    for (float x = 0; x < VoxelData.WorldSide; x += VoxelData.WorldStep)
                    {
                        Vector3 min_b = new Vector3(x, y, 0);
                        Vector3 max_b = new Vector3(x + 1, y + 1, 256);

                        if(fstm.IsVisible(min_b, max_b))
                        {
                            tiles.Add(min_b.Xy);
                        }
                    }
