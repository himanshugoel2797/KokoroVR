...
namespace KokoroVR
{
    ...
    public class VRClient : IDisposable
    {
        ...
        private VRClient(CVRSystem vr, ExperienceKind expKind)
        {
            ...
            {
                handle = (IntPtr)LeftColorTexture,
            ...
            {
                handle = (IntPtr)RightColorTexture,
                eColorSpace = EColorSpace.Auto,
                eType = ETextureType.OpenGL
            };
        ...
        public void GetControllerMesh(ulong activeOrigin, MeshGroup group, out Mesh[] meshes, out Texture[] texs)
        {
            ...
            for (uint q = 0; q < comp_cnt; q++)
            {
                ...
                if (!renderModels.ContainsKey((int)group + "_" + comp_name_bldr.ToString() + "_" + str))
                {
                    unsafe
                    {
                        ...
                        else
                        {
                            ...
                            meshes_l = m;
                            renderModels[(int)group + "_" + comp_name_bldr.ToString() + "_" + str] = new renderModel()
                            {
                                mesh = m,
                ...
                {
                    texs_l = renderTextures[renderModels[(int)group + "_" + comp_name_bldr.ToString() + "_" + str].texId];
                    meshes_l = renderModels[(int)group + "_" + comp_name_bldr.ToString() + "_" + str].mesh;
                }
